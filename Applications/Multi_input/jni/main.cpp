...
createFakeMultiDataGenerator(unsigned int batch_size,
  ...
  // Fix input order to match compilation order: input1 (1:4:2) comes before
  // input0 (1:1:2)
  UserDataType train_data(new nntrainer::util::MultiDataLoader(
    {{batch_size, 1, 4, 2},
     {batch_size, 1, 1, 2}}, // input1, input0 order (compilation order)
...
void runInferenceAndPrintSamples(const ModelHandle &model,
  ...
  const int num_samples_to_test = std::min(8, (int)batch_size);
  bool last = false;
  ...
  for (int sample_idx = 0; sample_idx < num_samples_to_test && !last;
    ...
    // Print input0 (1:1:2) - second input in compilation order
    std::cout << "Input0: [";
    for (int i = 0; i < 2; ++i) {
    ...
    // Print input1 (1:4:2) - first input in compilation order
    std::cout << "Input1: [";
    for (int i = 0; i < 8; ++i) {
    ...
    // Output compilation order: output_2 comes before output_1
    if (outputs.size() >= 2) {
      std::cout << "Label0: " << label_tensors[0][0]
                << ", Predicted0: " << outputs[1][0]
                << std::endl; // output_1 is second
      std::cout << "Label1: " << label_tensors[1][0]
                << ", Predicted1: " << outputs[0][0]
                << std::endl; // output_2 is first
    } else {
      std::cout << "Error: Expected 2 outputs but got " << outputs.size()
                << std::endl;
    }

    // Don't clear outputs - model manages this memory and will clean it up
    // The vector will be destroyed automatically when it goes out of scope
  }

  std::cout << "\n----------------------------------------" << std::endl;
  std::cout << "Inference completed!" << std::endl;

  // No manual cleanup needed - std::vector handles memory automatically
}
