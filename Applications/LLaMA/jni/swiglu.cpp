 ...
 */
float swish(float x) {
  return x / (1 + nntrainer::exp_util(-x));  
}
} // namespace ActivationOp
    ...
    for (int c = 0; c < (int)in.channel(); c++) {
      for (int h = 0; h < (int)in.height(); h++) {
        for (int w = 0; w < (int)in.width(); w++) {
          ...
          int out_idx = in.batch() * b + in.channel() * c + in.height() * h + w / 2;          
          out.getData()[out_idx] = in.getValue(in_idx) * ActivationOp::swish(in.getValue(in_idx + 1));
        }
      }
    }
  }
