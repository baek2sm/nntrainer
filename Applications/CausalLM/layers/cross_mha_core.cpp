...
/**
 * @brief constructor of CrossMHACoreLayer
 */
CrossMHACoreLayer::CrossMHACoreLayer() :
  mha_core_props(
    ...
    props::AttnLogitSoftcapping(), props::IsCausal(false)),
...
void CrossMHACoreLayer::incremental_forwarding(nntrainer::RunLayerContext &context,
  ...
  unsigned int query_from = _from;
  unsigned int query_to = _to;

  if (query_to >= max_timestep && !query_from) {
  ...
  auto get_input_step_dim = [](const ml::train::TensorDim &dim) {
    auto step_dim = dim;
    step_dim.batch(1);
    step_dim.height(dim.height());
    return step_dim;
  };

  // util fn to compute cache tensor dimension for KV cache update.
  auto get_cache_step_dim = [](const ml::train::TensorDim &dim,
                               unsigned int cache_from,
                               unsigned int cache_to) {
    auto step_dim = dim;
    step_dim.batch(1);
    step_dim.height(cache_to - cache_from);
    return step_dim;
  };
  ...
  const unsigned int cache_from = 0;
  const unsigned int cache_to = key_dim.height();

  NNTR_THROW_IF(cache_to > cache_key_dim.height() ||
                  cache_to > cache_value_dim.height(),
                std::invalid_argument)
    << "cross attention key/value input sequence is bigger than cache size";
  ...
  ml::train::TensorDim cache_key_step_dim =
    get_cache_step_dim(cache_key_dim, cache_from, cache_to);
  ml::train::TensorDim cache_value_step_dim =
    get_cache_step_dim(cache_value_dim, cache_from, cache_to);
    ...
    if (query_step.getDataType() == ml::train::TensorDim::DataType::FP32) {
      ...
      if (use_sink) {
        one_batch_incremental_forwarding(
          batch, query_from, cache_from, cache_to, Q_step, K_step, V_step,
          O_step, cache_key,
        ...
        one_batch_incremental_forwarding(
          batch, query_from, cache_from, cache_to, Q_step, K_step, V_step,
          O_step, cache_key, cache_value, cache_key_dim, cache_key_step_dim,
      ...
      if (use_sink) {
        one_batch_incremental_forwarding(
          batch, query_from, cache_from, cache_to, query_step, key_step,
          value_step, output_step, cache_key, cache_value, cache_key_dim,
    ...
    for (unsigned int batch = 1; batch < batch_size; ++batch) {
      nntrainer::Tensor cache_key_nth_step = cache_key.getSharedDataTensor(
        ...
        batch * cache_key_dim.getFeatureLen() +
          cache_from * cache_key_dim.width(),
        true);
      nntrainer::Tensor cache_value_nth_step =
        cache_value.getSharedDataTensor(
 ...
 *
 */
void CrossMHACoreLayer::compute_kcaches(
...
void CrossMHACoreLayer::one_batch_incremental_forwarding(
  const unsigned int batch, const unsigned int query_from,
  const unsigned int cache_from, const unsigned int cache_to,
  ...
  apply_rotary_emb_tensor_v2(query_step, query_step, head_dim, query_from,
                             false);
  ...
  unsigned int kv_seq_len = cache_to;
  unsigned int attention_from =
    is_causal ? query_from : (kv_seq_len > q_seq_len ? kv_seq_len - q_seq_len
                                                     : 0);
  ...
  compute_kcaches(query_step, b_cached_key, out_, attention_from, q_seq_len,
                  num_heads_Q, gqa_size, head_dim, pool);

  softmax_triangle(out_, q_seq_len, num_heads_Q, attention_from, pool);

  compute_fp16vcache_transposed(out_, b_cached_value, attention_output_step,
                                attention_from, num_heads_KV, gqa_size,
                                head_dim, kv_seq_len, pool);
}
 ...
 */
void CrossMHACoreLayer::precompute_freqs(int head_dim, unsigned int seq_len,
...
void CrossMHACoreLayer::_compute_default_parameters(int head_dim, float theta) {
...
void CrossMHACoreLayer::_compute_yarn_parameters(int head_dim, float theta) {
...
void CrossMHACoreLayer::apply_rotary_emb_tensor_v2(nntrainer::Tensor &in,
...
void CrossMHACoreLayer::softmax_triangle(nntrainer::Tensor &qk_out, size_t row,
...
void CrossMHACoreLayer::compute_fp16vcache_transposed(
...
void CrossMHACoreLayer::updateTensorsByInputDimensions(
  ...
  unsigned int height = query_dim.height();
  unsigned int &max_timestep =
    std::get<nntrainer::props::MaxTimestep>(mha_core_props).get();
...
size_t CrossMHACoreLayer::calc_attn_index(size_t i) { return (i * (i + 1)) / 2; };
