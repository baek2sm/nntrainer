#include "vocab_selection.h"
#include <algorithm>
#include <tensor.h>
#include <tensor_dim.h>

causallm::VocabSelection::VocabSelection(LshType lshType, size_t lshChoices,
                                         size_t hiddenSize, size_t vocabCnt)
    : hiddenSize(hiddenSize), vocabCnt(vocabCnt), lshBlockNum(0), lshBits(0),
      lshChoices(lshChoices), lshType(lshType) {}

causallm::VocabSelection::~VocabSelection() {}

causallm::VocabSelectionNNTrainer::VocabSelectionNNTrainer(
    ...
    for (size_t j = 0; j < vocabCnt; ++j) {
      ...
      for (size_t k = 0; k < actualSize; ++k) {
        if (weights.getDataType() == nntrainer::TensorDim::DataType::FP32) {
          ...
          d[k] = weights.getValue<_FP16>(0, 0, i * lshBlockSize + k, j) > 0 ? 1
                                                                            : 0;
        }
      }
      for (size_t k = actualSize; k < lshBlockSize; ++k) {
...
causallm::VocabSelectionNNTrainer::getVocabs(const nntrainer::Tensor &input) {
