...
void MHACoreLayer::incremental_forwarding(nntrainer::RunLayerContext &context,
  ...
  if (!is_cross_attention && to >= max_timestep) {
    // initial forwarding
    if (!_from) {
    ...
    for (unsigned int batch = 0; batch < batch_size; ++batch) {
      ...
      if (use_sink) {
        one_batch_incremental_forwarding(
          batch, query_step, output_step, cache_key, cache_value,
...
#pragma omp parallel for schedule(static)
      ...
      if (!is_causal && non_causal_context_len > 0)
        seq = sequence_len;
...
void MHACoreLayer::one_batch_incremental_forwarding(
  ...
  const unsigned int query_seq_len = query_step.height();
  ...
  compute_kcaches(query_step, b_cached_key, out_, 0, query_seq_len, num_heads_Q,
                  gqa_size, head_dim, pool, context_len);
  softmax_triangle(out_, query_seq_len, num_heads_Q, 0, pool, context_len);
  compute_fp16vcache_transposed(out_, b_cached_value, attention_output_step, 0,
                                num_heads_KV, gqa_size, head_dim,
                                query_seq_len, pool, context_len);
}
...
void MHACoreLayer::one_batch_incremental_forwarding(
  ...
  compute_kcaches(query_step, b_cached_key, out_, 0, query_seq_len, num_heads_Q,
                  gqa_size, head_dim, pool, context_len);
  softmax_triangle(out_, query_seq_len, num_heads_Q, 0, pool, sink_step,
                   context_len);
  compute_fp16vcache_transposed(out_, b_cached_value, attention_output_step, 0,
                                num_heads_KV, gqa_size, head_dim,
                                query_seq_len, pool, context_len);
...
void MHACoreLayer::compute_fp16vcache_transposed(
  ...
  const int cache_window =
    (!is_causal && non_causal_context_len > 0)
      ? context_len
      : static_cast<int>(local_window_size);
  const int fp16_non_causal_row_num =
    (!is_causal && non_causal_context_len > 0) ? context_len - 1 : to;
...
#pragma omp parallel for schedule(static)
    ...
    if ((to - from) != 1) {
      ...
      for (int i = 0; i < seq; ++i) {
        futures.push_back(pool.submit_task([=]() {
          ...
          _FP16 *out = output.getData<_FP16>() +
                       i * (num_cache_head * gqa_size * head_dim);
          int row_num = is_causal ? (to - seq + i) : fp16_non_causal_row_num;
      ...
      // Single token processing (common during generation)
      // Parallelize over KV heads for decoding since Q direction is always 1
      int row_num = is_causal ? to - 1 : fp16_non_causal_row_num;
