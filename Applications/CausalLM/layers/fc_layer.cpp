...
FCLayer::FCLayer()
    : LayerImpl(),
      custom_fc_props(nntrainer::props::Unit(), props::SmartReply()) {
  weight_idx.fill(std::numeric_limits<unsigned>::max());
}

void FCLayer::finalize(nntrainer::InitLayerContext &context) {
...
void FCLayer::forwarding(nntrainer::RunLayerContext &context, bool training) {
...
void FCLayer::incremental_forwarding(nntrainer::RunLayerContext &context,
...
void FCLayer::calcDerivative(nntrainer::RunLayerContext &context) {}

void FCLayer::setProperty(const std::vector<std::string> &values) {
...
nntrainer::Layer *create_fc_layer() {
  auto layer = new FCLayer();
  return layer;
}

void destroy_fc_layer(nntrainer::Layer *layer) { delete layer; }

extern "C" {
nntrainer::LayerPluggable ml_train_layer_pluggable{create_fc_layer,
                                                   destroy_fc_layer};
}
