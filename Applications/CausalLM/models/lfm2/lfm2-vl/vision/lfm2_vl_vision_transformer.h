...
public:
  ...
  /**
   * @brief Run inference on a pre-loaded float buffer (NCHW layout).
   *
   * @param image_data  Pointer to BATCH_SIZE * C * H * W float values.
   * @return Pair of (output_ptr, output_size_bytes). Valid until the
   *         next call or destruction.
   */
  std::pair<const float *, size_t> runBuffer(const float *image_data);
...
private:
  /** Buffer retaining the last runBuffer() output. */
  std::vector<float> last_buffer_output_;
};
