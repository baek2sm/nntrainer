 ...
 * CausalLM app architecture: the model is registered with the Factory and
 * driven by nntr_config.json (no YOLO_* env vars), so the standard
 * nntrainer_causallm binary runs it and nntr_quantize quantizes it — mirroring
...
/**
 * @brief Yolov11Transformer class — a Factory-registered detection model.
 */
class Yolov11Transformer : virtual public Transformer {

public:
  static constexpr const char *architectures = "YOLOv11ForDetection";

  /**
   * @brief Construct a Yolov11Transformer object.
   */
  Yolov11Transformer(json &cfg, json &generation_cfg, json &nntr_cfg) :
    Transformer(cfg, generation_cfg, nntr_cfg, ModelType::MODEL) {
    setupParameters(cfg, generation_cfg, nntr_cfg);
  }

  /**
   * @brief Destroy the Yolov11Transformer object.
   *
   ...
   */
  virtual ~Yolov11Transformer() = default;
