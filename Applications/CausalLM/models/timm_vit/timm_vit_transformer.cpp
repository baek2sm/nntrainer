...
void TimmViTTransformer::run(const WSTR prompt, bool do_sample,
  ...
  std::cout << "\n=================================\n";
  std::cout << "Image path: " << prompt << std::endl;
  ...
  // Print first 3 and last 3 values for debugging
  std::cout << "First 3 values: ";
  for (int i = 0; i < std::min(3, (int)image_data.size()); ++i) {
    std::cout << image_data[i] << " ";
  }
  std::cout << std::endl;

  std::cout << "Last 3 values: ";
  for (int i = std::max(0, (int)image_data.size() - 3);
       i < (int)image_data.size(); ++i) {
    std::cout << image_data[i] << " ";
  }
  std::cout << std::endl;

  std::cout << "Image loaded: " << image_data.size() << " pixels (expected "
            << input_size << ")" << std::endl;

  if (image_data.size() != input_size) {
    std::cerr << "Error: Image size mismatch! Expected " << input_size
