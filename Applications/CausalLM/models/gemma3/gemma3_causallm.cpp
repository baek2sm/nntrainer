...
json &Gemma3CausalLM::sanitizeConfig(json &cfg) {
  if (!cfg.contains("tie_word_embeddings")) {
    cfg["tie_word_embeddings"] = true; // Gemma models typically tie embeddings
  }

  return cfg;
}

json &Gemma3CausalLM::sanitizeGenerationConfig(json &gen_cfg, const json &cfg) {
  if (!gen_cfg.contains("bos_token_id")) {
    if (cfg.contains("bos_token_id")) {
      gen_cfg["bos_token_id"] = cfg["bos_token_id"];
    } else {
      gen_cfg["bos_token_id"] = 2; // Default fallback
    }
  }
  ...
  if (!gen_cfg.contains("eos_token_id")) {
    if (cfg.contains("eos_token_id")) {
      ...
      gen_cfg["eos_token_id"] =
        std::vector<unsigned int>{1}; // Default fallback
    }
  } else {
    // Ensure eos_token_id is a vector even if present in gen_cfg
...
void Gemma3CausalLM::setupParameters(json &cfg, json &generation_cfg,
                                     json &nntr_cfg) {
...
Gemma3CausalLM::createTransformerDecoderBlock(const int layer_id,
...
std::vector<LayerHandle>
Gemma3CausalLM::createAttention(const int layer_id, int seq_len, int n_heads,
  ...
  if (!layer_types.empty()) {
    ...
    // Fallback if layer_types not present
    window_size = SLIDING_WINDOW;
  }
  ...
  if (!layer_types.empty() && layer_id < layer_types.size()) {
    if (layer_types[layer_id] == "sliding_attention") {
      // Local attention layers use a smaller base frequency
      // Hardcoded based on config or logic mentioned in plan
      rope_theta = 10000.0f;
    }
  }
...
std::vector<LayerHandle> Gemma3CausalLM::createMlp(const int layer_id, int dim,
                                                   int hidden_dim,
                                                   std::string input_name) {
  std::vector<LayerHandle> layers;
