...
# layers. Weight order matches the #3963 base transformer.cpp / lfm2_causallm.cpp
...
#   ffn_up (w3), ffn_gate (w1), ffn_down (w2)
#
# Conv block (createConvBlock):
#   conv_norm (operator_norm), conv_in_proj, causal_conv1d, conv_out_proj,
#   ffn_norm, ffn_up (w3), ffn_gate (w1), ffn_down (w2)
#
# Note: ffn_up (w3 = up_proj) is stored before ffn_gate (w1 = gate_proj) because
# the ffn_up FC layer is constructed before ffn_gate in createMlp(). The swiglu
# layer uses input-index reordering ({1,0}) so the math is still correct.
#
...
def attn_layer_keys(index):
    ...
    return [
        ...
        (base + "feed_forward.w3.weight", True, None, None),   # ffn_up first (up_proj)
        (base + "feed_forward.w1.weight", True, None, None),   # ffn_gate second (gate_proj)
