...
void FFNLayer::forwarding(nntrainer::RunLayerContext &context,
                                     bool training) {
  return;
}

void FFNLayer::incremental_forwarding(
  ...
  // std::cout <<to-from <<" " << input_step_dim.height()<<" "<< input_step_dim.width()<<std::endl;
  // std::cout <<to-from <<" " << input_dim.height()<<" "<< input_dim.width()<<std::endl;
  // std::cout <<to-from <<" " << Qhidden_.getDim().height()<<" "<< Qhidden_.getDim().width()<<std::endl;
  // std::cout <<to-from <<" " << Khidden_.getDim().height()<<" "<< Khidden_.getDim().width()<<std::endl;
  // std::cout <<to-from <<" " << Vhidden_.getDim().height()<<" "<< Vhidden_.getDim().width()<<std::endl;      
...
void FFNLayer::calcDerivative(nntrainer::RunLayerContext &context) {
  return;
}

void FFNLayer::calcGradient(nntrainer::RunLayerContext &context) {
  return;
}

void FFNLayer::updateTensorsByInputDimensions(nntrainer::RunLayerContext &context,
