...
#include <custom_mha_core_v2_layer.h>
#include <fp16.h>
...
/**
 * @brief constructor of CustomMHACoreV2Layer
 */
CustomMHACoreV2Layer::CustomMHACoreV2Layer() :
  mha_core_props(
...
void CustomMHACoreV2Layer::finalize(nntrainer::InitLayerContext &context) {
  ...
  if (!std::get<props::Theta>(mha_core_props).empty()) {
    theta_ = std::get<props::Theta>(mha_core_props).get();
  }

  if (!std::get<props::SkipPrefill>(mha_core_props).empty()) {
    skip_prefill = std::get<props::SkipPrefill>(mha_core_props).get();
  }
  ...
  precompute_freqs(head_dim, max_timestep, theta_);
 ...
 * @date 2024-09-02
 */
void CustomMHACoreV2Layer::forwarding(nntrainer::RunLayerContext &context,
                                      bool training) {}
 ...
 */
void CustomMHACoreV2Layer::incremental_forwarding(
  ...
  auto ms =
    std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time)
      .count();
  auto ms0 = std::chrono::duration_cast<std::chrono::nanoseconds>(start_time -
                                                                  begin_time)
               .count();
  // std::cout << "***** Time: " << ms0 << ", " << ms<< " ns\n";

  // update KV cache
  if (!_from) {
    // std::cout <<"do just once"<<std::endl;
...
void CustomMHACoreV2Layer::compute_kcaches(
...
#pragma omp parallel for
    ...
    if (sequence_len == 1) {
      ...
      for (int n = 0; n < num_cache_head; ++n) {
        const _FP16 *in_ptr = in.getData<_FP16>() + n * group_size * head_dim;
        const _FP16 *cache_ptr = cache.getData<_FP16>() + n * head_dim;
        _FP16 *out_ptr = out.getData<_FP16>() + n * group_size;
      ...
      for (unsigned int i = seq_start; i < sequence_len; ++i) {
        ...
        const int tile_count = (row_cnt + tile_size - 1) / tile_size;
        for (int n = 0; n < num_cache_head; ++n) {
          const _FP16 *in_ptr = input_addr + n * group_size * head_dim;
          const _FP16 *cache_ptr = cache_addr + n * head_dim;
          _FP16 *out_ptr = output_addr + n * group_size;
          for (int tile_off = 0; tile_off < tile_count; ++tile_off) {
            nntrainer::compute_kcaches(in_ptr, cache_ptr, out_ptr, num_rows,
                                       num_cache_head, head_dim, group_size,
                                       tile_off, tile_size, local_window_size);
...
void CustomMHACoreV2Layer::one_batch_incremental_forwarding(
  ...
  // p_futures.push_back(pool.submit_task([&](){
  // cpu_set_t cpu_set;
  // CPU_ZERO(&cpu_set);
  ...
  // auto start_out_ = std::chrono::high_resolution_clock::now();
  ...
  auto start_kcache = std::chrono::high_resolution_clock::now();
  ...
  auto prepare_duration = std::chrono::duration_cast<std::chrono::nanoseconds>(
    end_prepare - start_prepare);
  auto kcache_duration = std::chrono::duration_cast<std::chrono::nanoseconds>(
    end_kcache - start_kcache);
  auto softmax_duration = std::chrono::duration_cast<std::chrono::nanoseconds>(
    end_softmax - start_softmax);
  auto vcache_duration = std::chrono::duration_cast<std::chrono::nanoseconds>(
    end_vcache - end_softmax);

  auto ms_ = std::chrono::duration_cast<std::chrono::nanoseconds>(start_kcache -
                                                                  end_prepare)
               .count();

  //   std::cout <<to << " -- P: "<<prepare_duration.count() << " out_: "<<ms_<<
  //   " K: "<<kcache_duration.count() << " S: "<< softmax_duration.count()<< "
  //   V: "<< vcache_duration.count()<<std::endl;
}
 ...
 */
void CustomMHACoreV2Layer::precompute_freqs(int head_dim, unsigned int seq_len,
                                            float theta) {
  if (!rotary_embedding_precomputed) {
    rotary_embedding_precomputed = true;
    ...
    // cos / sin
    freqs_cos.assign(seq_len, std::vector<float>(head_dim, 0));
    freqs_sin.assign(seq_len, std::vector<float>(head_dim, 0));
    ...
    for (unsigned int i = 0; i < seq_len; ++i) {
      ...
      nntrainer::calc_trigonometric_vals_dup(
        half_, thetas.data(), freqs_cos[i].data(), freqs_sin[i].data(), i);
#else
      for (unsigned int j = 0; j < half_; ++j) {
        float angle = i * thetas[j];
        freqs_cos[i][j] = std::cos(angle);
        freqs_cos[i][j + half_] = std::cos(angle); // repeated 2 times

        freqs_sin[i][j] = std::sin(angle);
        freqs_sin[i][j + half_] = std::sin(angle); // repeated 2 times
      }
#endif
    }
  }
...
void CustomMHACoreV2Layer::apply_rotary_emb_tensor_v2(nntrainer::Tensor &in,
...
void CustomMHACoreV2Layer::softmax_triangle(nntrainer::Tensor &qk_out,
...
void CustomMHACoreV2Layer::compute_fp16vcache_transposed(
...
#pragma omp parallel for
      for (int i = 0; i < seq; ++i) {
        ...
        nntrainer::compute_fp16vcache_fp32_transposed(
          to - seq + i, input, vcache.getData<uint16_t>(), out, num_cache_head,
      ...
      for (int i = 0; i < seq; ++i) {
        ...
        for (int n = 0; n < num_cache_head; ++n) {
          ...
          nntrainer::compute_fp16vcache_transposed(
            to - seq + i, in_ptr, vcache_ptr, out_ptr, num_cache_head, gqa_size,
 ...
 * @brief rotary embedding-related member function
 */
void CustomMHACoreV2Layer::apply_rotary_emb_tensor(nntrainer::Tensor &in,
                                                   unsigned int dim,
                                                   unsigned int from) {
...
size_t CustomMHACoreV2Layer::calc_attn_index(size_t i) {
  size_t local_window_size =
    std::get<props::LocalWindowSize>(mha_core_props).get();
...
void CustomMHACoreV2Layer::setBatch(nntrainer::RunLayerContext &context,
                                    unsigned int batch) {
...
void CustomMHACoreV2Layer::updateTensorsByInputDimensions(
...
void CustomMHACoreV2Layer::calcDerivative(nntrainer::RunLayerContext &context) {
}

void CustomMHACoreV2Layer::calcGradient(nntrainer::RunLayerContext &context) {}

void CustomMHACoreV2Layer::exportTo(
...
void CustomMHACoreV2Layer::setProperty(const std::vector<std::string> &values) {
...
nntrainer::Layer *create_mha_core_layer() {
  auto layer = new CustomMHACoreV2Layer();
  return layer;
}
