...
void CustomMultiHeadAttentionLayer::finalize(
  ...
  weight_idx[AttentionParams::projected_key] = context.requestTensor(
    projected_key_dim, "projected_key", nntrainer::Initializer::NONE,
  ...
  if (dropout_rate > epsilon) {
    ...
    weight_idx[AttentionParams::dropout_mask] = context.requestTensor(
      dropout_mask_dim, "dropout_mask", nntrainer::Initializer::NONE,
      false, nntrainer::TensorLifespan::ITERATION_LIFESPAN);
  }
...
#define _MASK_NUM -1e10
  ...
  if (to - from != 1) {
    attention_output_step.reshape(
      ml::train::TensorDim({1, num_heads, to - from, projected_value_dim_prop}));

    attention_output_step = attention_output_step.transpose("1:0:2");
  }

  if (fc_weight.getDataType() == nntrainer::TensorDim::DataType::BCQ){
    attention_output_step.reshape(ml::train::TensorDim(
...
void CustomMultiHeadAttentionLayer::incremental_forwarding(
  ...
  empty_tensor.getDim().setTensorType(value.getDim().getTensorType());
...
nntrainer::Layer *create_multi_head_attention_layer() {
  auto layer = new CustomMultiHeadAttentionLayer();
  return layer;
}

void destroy_multi_head_attention_layer(nntrainer::Layer *layer) {
  delete layer;
}

extern "C" {
nntrainer::LayerPluggable ml_train_layer_pluggable{
  create_multi_head_attention_layer, destroy_multi_head_attention_layer};
}
