...
void CustomSwiGLULayer::finalize(nntrainer::InitLayerContext &context) {

  if (!std::get<props::SkipPrefill>(custom_swiglu_props).empty()) {
    skip_prefill = std::get<props::SkipPrefill>(custom_swiglu_props).get();
  }

  context.setOutputDimensions({context.getInputDimensions()[0]});
}

void CustomSwiGLULayer::forwarding(nntrainer::RunLayerContext &context,
                                   bool training) {}

void CustomSwiGLULayer::incremental_forwarding(
  ...
  bool smart_reply = std::get<props::SmartReply>(custom_swiglu_props).get();
      ...
      for (unsigned int c = 0; c < in1.channel(); c++) {
        for (unsigned int h = from; h < to; h++) {
            nntrainer::swiglu(in1.width(),
                              out.getData<float>() + out.getIndex(b, c, h,
                              0), in1.getData<float>() + in1.getIndex(b, c,
                              h, 0), in2.getData<float>() + in2.getIndex(b,
                              c, h, 0));          
        }
      }
...
void CustomSwiGLULayer::updateTensorsByInputDimensions(
...
void CustomSwiGLULayer::calcDerivative(nntrainer::RunLayerContext &context) {
...
nntrainer::Layer *create_swiglu_layer() {
  auto layer = new CustomSwiGLULayer();
  return layer;
}
