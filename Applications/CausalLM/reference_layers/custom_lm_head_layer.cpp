...
#include <custom_lm_head_layer.h>
...
CustomLMHeadLayer::CustomLMHeadLayer() :
  LayerImpl(),
...
void CustomLMHeadLayer::finalize(nntrainer::InitLayerContext &context) {
  ...
  if (use_vocab_selection) {
    ...
    weight_idx[LMHeadParams::candidate_weight] =
    ...
    weight_idx[LMHeadParams::candidate_hidden_step] =
...
void CustomLMHeadLayer::forwarding(nntrainer::RunLayerContext &context,
                                   bool training) {
  // NYI
}

#ifdef ENABLE_FP16
void CustomLMHeadLayer::initVocabSelection(
...
void CustomLMHeadLayer::incremental_forwarding(
...
void CustomLMHeadLayer::calcDerivative(nntrainer::RunLayerContext &context) {}

void CustomLMHeadLayer::setProperty(const std::vector<std::string> &values) {
...
nntrainer::Layer *create_custom_lm_head_layer() {
  auto layer = new CustomLMHeadLayer();
  return layer;
}
