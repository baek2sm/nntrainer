...
const std::map<std::string, DataType> dtype_str_map = {
  ...
  {"Q4_0", DataType::Q4_0}, {"Q8_0", DataType::Q8_0}, {"Q6_K", DataType::Q6_K},
...
void registerAllModels() {
  auto &factory = quick_ai::Factory::Instance();

  factory.registerModel("LlamaForCausalLM", [](json cfg, json generation_cfg,
                                               json nntr_cfg) {
    return std::make_unique<quick_ai::CausalLM>(cfg, generation_cfg, nntr_cfg);
  });
  factory.registerModel("Qwen2ForCausalLM",
                        [](json cfg, json generation_cfg, json nntr_cfg) {
                          return std::make_unique<quick_ai::Qwen2CausalLM>(
                            cfg, generation_cfg, nntr_cfg);
                        });
  factory.registerModel("Qwen2Embedding",
                        [](json cfg, json generation_cfg, json nntr_cfg) {
                          return std::make_unique<quick_ai::Qwen2Embedding>(
                            cfg, generation_cfg, nntr_cfg);
                        });
  factory.registerModel("Qwen3ForCausalLM",
                        [](json cfg, json generation_cfg, json nntr_cfg) {
                          return std::make_unique<quick_ai::Qwen3CausalLM>(
                            cfg, generation_cfg, nntr_cfg);
                        });
  factory.registerModel("Qwen3MoeForCausalLM",
                        [](json cfg, json generation_cfg, json nntr_cfg) {
                          return std::make_unique<quick_ai::Qwen3MoECausalLM>(
                            cfg, generation_cfg, nntr_cfg);
                        });
  factory.registerModel("Qwen3SlimMoeForCausalLM", [](json cfg,
                                                      json generation_cfg,
                                                      json nntr_cfg) {
    return std::make_unique<quick_ai::Qwen3SlimMoECausalLM>(cfg, generation_cfg,
                                                            nntr_cfg);
  });
#if !defined(_WIN32)
  ...
  factory.registerModel("Qwen3Embedding",
                        [](json cfg, json generation_cfg, json nntr_cfg) {
                          return std::make_unique<quick_ai::Qwen3Embedding>(
                            cfg, generation_cfg, nntr_cfg);
                        });
  factory.registerModel("GptOssForCausalLM",
                        [](json cfg, json generation_cfg, json nntr_cfg) {
                          return std::make_unique<quick_ai::GptOssForCausalLM>(
                            cfg, generation_cfg, nntr_cfg);
                        });
#if !defined(_WIN32)
  factory.registerModel(
  ...
  factory.registerModel("Gemma3ForCausalLM",
                        [](json cfg, json generation_cfg, json nntr_cfg) {
                          return std::make_unique<quick_ai::Gemma3CausalLM>(
                            cfg, generation_cfg, nntr_cfg);
                        });
  factory.registerModel("Gemma4ForCausalLM",
                        [](json cfg, json generation_cfg, json nntr_cfg) {
                          return std::make_unique<quick_ai::Gemma4CausalLM>(
                            cfg, generation_cfg, nntr_cfg);
                        });
  factory.registerModel("EmbeddingGemma",
                        [](json cfg, json generation_cfg, json nntr_cfg) {
                          return std::make_unique<quick_ai::EmbeddingGemma>(
                            cfg, generation_cfg, nntr_cfg);
                        });
...
#endif
  // Vision models (conv-based, tokenizer-free). Mirrors nntr_quick_ai
...
void addSentenceTransformerLayerDtypes(std::map<std::string, DataType> &map,
  ...
  json modules_json = quick_ai::LoadJsonFile(modules_config_path.string());
  ...
  try {
    ...
    json generation_cfg = json::object();
    {
      ...
      if (std::filesystem::exists(gen_cfg_path))
        generation_cfg = quick_ai::LoadJsonFile(gen_cfg_path);
    }
    ...
    if (!target_config_path.empty()) {
      ...
      json target_cfg = quick_ai::LoadJsonFile(target_config_path);
    ...
    if (output_dir == model_path) {
      std::cout << "  1. Rename nntr_config_quantized.json to "
                   "nntr_config.json\n";
      std::cout << "  2. nntr_quick_ai " << model_path << "\n";
    } else {
      std::cout << "  nntr_quick_ai " << output_dir << "\n\n";
    }
