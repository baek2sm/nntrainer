...
inline Tensor convGelu(const std::string &name, int in_ch, int out_ch, int k,
  ...
  if (out_ch % 32 == 0 && (in_ch / groups * k * k) % 32 == 0)
    conv_props.push_back(nntrainer::withKey("weight_dtype", "Q8_0"));
 ...
 *
 * Forward:
 *   x = x + layer_scale_1(token_mixer(norm(x)))
 *   x = x + layer_scale_2(mlp(x))
 *
 * norm: BatchNorm2d(512)
 * token_mixer: qkv (1x1 conv, 512->1536, no bias), attention, proj (1x1 conv,
 * 512->512, bias) Attention: 16 heads, head_dim=32, scale=1/sqrt(32) mlp: dw
 * 7x7 conv + BN, fc1(1x1, 512->2048)+GELU, fc2(1x1, 2048->512) layer_scale_1,
 * layer_scale_2: elementwise multiply by gamma [512,1,1]
 *
 * @param name  Layer-name prefix
...
inline Tensor attentionBlock(const std::string &name, int ch, Tensor input) {
  // norm (BatchNorm2d) — not fused, needs BN params loaded
  LayerHandle norm(createLayer("batch_normalization",
                               {nntrainer::withKey("name", name + "/attn_norm"),
                                nntrainer::withKey("epsilon", 1e-5)}));
  auto normed = norm(input);

  // qkv: 1x1 conv (512 -> 1536, no bias)
  auto qkv = conv1x1Only(name + "/qkv", ch, ch * 3, normed, true);
...
inline Tensor buildBackbone(Tensor xIn) {
  ...
  // === Stage 0: 2x RepMixerBlock(64) ===
  x = repMixerBlock("s0b0", 64, x);
  x = repMixerBlock("s0b1", 64, x);

  // === Stage 1: downsample + 2x RepMixerBlock(128) ===
  x = downsampleBlock("s1_down", 64, 128, x); // -> [1,128,40,40]
  return x;
}
