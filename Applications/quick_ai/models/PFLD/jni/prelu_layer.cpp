...
void PReLULayer::forwarding(nntrainer::RunLayerContext &context,
  ...
  if (std::getenv("NNTR_DEBUG_FORWARD") != nullptr) {
    std::cout << "[PReLULayer forwarding] name: " << context.getName()
              << " x=" << (void*)x << " y=" << (void*)y << " a=" << (void*)a << std::endl;
    if (x) std::cout << "  input[0..4]: " << x[0] << ", " << x[1] << ", " << x[2] << ", " << x[3] << ", " << x[4] << std::endl;
    if (a) std::cout << "  alpha[0..4]: " << a[0] << ", " << a[1] << ", " << a[2] << ", " << a[3] << ", " << a[4] << std::endl;
  }
