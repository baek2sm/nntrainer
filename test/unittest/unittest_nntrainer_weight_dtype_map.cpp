...
nntrainer::InitLayerContext makeCtx(const std::string &weight_dt_default,
  ...
  std::vector<nntrainer::TensorDim> in_dims = {nntrainer::TensorDim(
    1, 1, 1, 32,
  ...
  return nntrainer::InitLayerContext(
    ...
    /*max_norm=*/0.0f, {"NCHW", weight_dt_default, activation_dt_default},
...
TEST(WeightDtypeMap, empty_map_returns_fallback) {
  ...
  EXPECT_EQ(ctx.getDataTypeForRole("weight", ctx.getWeightDataType()),
            DT::Q8_0);
...
TEST(WeightDtypeMap, single_role_override) {
  ...
  // weight -> Q4_0 (from map), bias -> FP32 (fallback), gamma -> Q8_0
  // (fallback)
  EXPECT_EQ(ctx.getDataTypeForRole("weight", ctx.getWeightDataType()),
            DT::Q4_0);
  EXPECT_EQ(ctx.getDataTypeForRole("bias", ctx.getActivationDataType()),
