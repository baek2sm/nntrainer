...
TEST(Q8_0_Tensor, quantize_dequantize_round_trip) {
  ...
  nntrainer::TensorDim fp32_dim(1, 1, H, W,
                                {nntrainer::Tformat::NCHW,
  ...
  for (unsigned int i = 0; i < H * W; ++i) {
    src_data[i] = static_cast<float>(i % 32) * 1.5f - 24.0f; // Range: -24 to +22.5
  }

  // Quantize FP32 -> Q8_0
  nntrainer::GgmlQuantizer q(nntrainer::QScheme::Q8_0);
  nntrainer::Tensor q_tensor = q.quantize(src, nntrainer::TensorDim::DataType::Q8_0);
  EXPECT_EQ(q_tensor.getDataType(), nntrainer::TensorDim::DataType::Q8_0);

  // Dequantize Q8_0 -> FP32
  nntrainer::Tensor deq_tensor = q.dequantize(q_tensor, nntrainer::TensorDim::DataType::FP32);
  ...
  for (unsigned int b = 0; b < NUM_BLOCKS; ++b) {
    ...
    // Rounding error of 8-bit can be up to 0.5 * step_size (which is max_abs_val / 127.0).
    // Allowing a very small tolerance for float precision.
    for (int l = 0; l < 32; ++l) {
      float diff = std::abs(src_t_data[b * 32 + l] - deq_data[b * 32 + l]);
      EXPECT_LE(diff, error_bound + 1e-5f)
        << "Excessive error at block " << b << ", element " << l
        << ": original=" << src_t_data[b * 32 + l] << ", reconstructed=" << deq_data[b * 32 + l];
    }
  }
}
...
TEST(Q8_0_Tensor, indirect_conv_q80_vs_fp16) {
  ...
  nntrainer::TensorDim in_dim(B, C, IH, IW, {nntrainer::Tformat::NCHW, nntrainer::TensorDim::DataType::FP16});
  ...
  // 1. Run indirect conv on FP16 input (internally quantizes to Q8_0 on the fly)
  in_fp16.convQ4_0Indirect(filter_q40, out_fp16, geom);

  // 2. Run indirect conv directly on pre-quantized Q8_0 input (skips on-the-fly quantization)
  in_q80.convQ4_0Indirect(filter_q40, out_q80, geom);

  // Verify that the two outputs are mathematically identical (or extremely close within float tolerance)
  ...
  for (unsigned int i = 0; i < out_dim.getDataLen(); ++i) {
    float diff = std::abs(static_cast<float>(fp16_out[i]) - static_cast<float>(q80_out[i]));
