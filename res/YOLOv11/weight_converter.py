...
class Pack:
    ...
    def c3k2(self, pt, nn):
        ...
        # C3k blocks have cv3 only when c3k=True (larger models); smaller
        # models use a plain Bottleneck (cv1+cv2 only, no cv3).
...
def main(weights, out):
    try:
        ...
        sd = model.state_dict()
        print("Loaded as TorchScript JIT model (already fused)")
    except Exception as e:
        print(f"Failed to load as JIT, falling back to ultralytics: {e}")
