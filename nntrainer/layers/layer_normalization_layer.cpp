...
nntrainer::Tensor deviation_step = deviation.getSharedDataTensor(dev_step_dim, dev_offset, true);

TensorDim inv_step_dim = inv_std_dev.getDim();
inv_step_dim.height(step_h); 
Tensor inv_std_step =
  inv_std_dev.getSharedDataTensor(inv_step_dim, from * inv_std_dev.getDim().width(), true);
