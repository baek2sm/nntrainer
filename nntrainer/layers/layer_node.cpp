...
LayerNode::LayerNode(std::unique_ptr<nntrainer::Layer> &&l) :
  ...
  layer_node_props(
    new PropsType(props::Name(), props::Distribute(), props::Trainable(), {},
                  ...
                  props::ComputeEngine(), props::TensorDataType())),
...
InitLayerContext LayerNode::finalize(const std::vector<TensorDim> &input_dims,
  ...
  if (!std::get<props::TensorDataType>(*layer_node_props).empty()) {
    auto weight_type_prop =
      std::get<props::TensorDataType>(*layer_node_props).get();
    auto weight_type =
      str_converter<enum_class_prop_tag,
                    nntrainer::TensorDataTypeInfo>::to_string(weight_type_prop);
    tensor_type[1] = weight_type;
