...
void SliceLayer::forwarding_operation(const Tensor &input, Tensor &output) {
  if (output.getDataType() == ml::train::TensorDim::DataType::Q8_0_TW) {
    // Q8_0_TW carries a flat int8 payload (1 byte/element) in the same
    // NCHW/NHWC element ordering as any other dtype; sliceForwardT is
    // size-generic via sizeof(T), so an int8 copy of the selected sub-range is
    // exact. Calibration keeps the scale unchanged across the slice (pure
    // passthrough view).
